...
const DARK_COLORS: ColorPalette = {
  ...
  core: "#dddfe3",
  centerGlow:
    "radial-gradient(circle,rgba(0,85,255,0.20) 0%,rgba(0,85,255,0.05) 40%,transparent 70%)",
};
...
const LIGHT_COLORS: ColorPalette = {
  ...
  core: "#0055FF",
  centerGlow:
    "radial-gradient(circle,rgba(0,85,255,0.08) 0%,rgba(0,85,255,0.03) 40%,transparent 70%)",
};
...
function CenterGlow({ colors }: { colors: ColorPalette }) {
  return (
    <motion.div
      className="pointer-events-none absolute top-1/2 left-1/2 h-[200px] w-[200px] -translate-x-1/2 -translate-y-1/2 blur-[30px]"
      style={{ backgroundImage: colors.centerGlow }}
...
export function CircuitDivider({
  ...
  // Rise-zoom parallax
  const riseZoomY = useTransform(scrollYProgress, [0, 0.5, 1], [60, 0, -60]);
  const riseZoomScale = useTransform(
    scrollYProgress,
    [0, 0.5, 1],
    [0.75, 1, 0.75],
  );
  ...
  const parallaxStyle = hasParallax
    ? { y: riseZoomY, scale: riseZoomScale }
    : undefined;

  return (
    <div
      ref={scrollRef}
      style={{ background: `linear-gradient(to bottom, transparent, ${from} 30%, ${to} 70%, transparent)` }}
      className={cn(
        // -my chevauche le padding des sections : fond la transition sans ajouter de hauteur
        "relative -my-[116px] h-[400px] overflow-hidden",
        className,
      )}
    >
      <motion.div

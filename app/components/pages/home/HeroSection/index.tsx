					...
					<div className='w-full lg:max-w-[550px]'>
						...
						<div className='mt-6 lg:mt-10 flex flex-col md:flex-row items-start md:items-center gap-3 sm:gap-5'>
							...
							{homeInfo.socials.map((item, index) => {
								return <li key={item.name}><a href={item.url} target="_blank" rel="noopener noreferrer" aria-label={`Voir mon profil ${item.name}`} className="inline-flex p-1 rounded-md text-pale-sky-400 hover:text-blue-ribbon-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-ribbon-300"><CMSIcon icon={item.iconSvg} /></a></li>
							})}
							</ul>
						</div>
					</div>
